...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction, client: BotClient): Promise<unknown> {
        ...
        if (!ownerId) {
            client.error(
                "[DownloadLimit] Developer ID is not configured as OWNER_ID in environment variables!"
            )
        ...
        if (interaction.user.id !== ownerId) {
            client.debug(
                `[DownloadLimit] Denied access to user ${interaction.user.tag} (${interaction.user.id})`
            )
            return interaction.reply({
