...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("deployglobal").setDescription("Deploy/refresh slash commands globally")
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("deploylocal")
  ...
  async execute(interaction: ChatInputCommandInteraction, client: BotClient): Promise<unknown> {
    ...
    try {
      ...
      if (subcommand === "deploylocal") {
        ...
        await rest.put(Routes.applicationGuildCommands(interaction.client.user.id, targetGuildId), {
          body: commands,
        })
