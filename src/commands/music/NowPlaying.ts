...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction, client: BotClient): Promise<unknown> {
        ...
        if (localPlayerState && localPlayerState.isPlaying && localPlayerState.trackTitle) {
            const elapsedMs = localPlayerState.startedAt
                ? Date.now() - localPlayerState.startedAt
                : 0
        ...
        const embed = new EmbedBuilder()
            ...
            .setThumbnail(
                track.info.artworkUrl ||
                    (track.info.identifier && track.info.sourceName === "youtube"
                        ? `https://img.youtube.com/vi/${track.info.identifier}/hqdefault.jpg`
                        : null)
            )
            .setFooter({
                text: `Playing via Lavalink node: ${lavalinkPlayer.node?.id ?? "Unknown"}`,
            })
            .setTimestamp()
