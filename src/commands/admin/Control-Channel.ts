    ...
    if (subcommand === "set") {
      if (!channel || channel.type !== ChannelType.GuildText) {
        ...
        return interaction.reply({
          content: "Control channel must be a server text channel. Run this command from that channel.",
        })
      }
      ...
      if (!botUser) {
        return interaction.reply({ content: "Bot user is not available yet." })
      }
      ...
      if (oldChannelId && oldMessageId) {
        ...
        try {
          const oldChannel = await client.channels.fetch(oldChannelId).catch((): null => null)
          if (oldChannel && "messages" in oldChannel) {
            const oldMessage = await oldChannel.messages.fetch(oldMessageId).catch((): null => null)
            if (oldMessage) {
      ...
      if (controlMessageId) {
        ...
        try {
          const controlChannel = await client.channels.fetch(controlChannelId).catch((): null => null)
