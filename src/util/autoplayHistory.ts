 ...
 */
export function isPlausibleAutoplayMusicTrack(info: TrackInfo | UnresolvedTrackInfo | undefined) {
...
function latinWordBlob(s: string | undefined) {
...
function compositionMatchFromCores(
    ca: string,
    cb: string,
    authorA: string | undefined,
    authorB: string | undefined,
    opts: { strictShortCrossArtist?: boolean } = {}
) {
...
export function matchesCatalogCandidate(
    ...
    catalogTitle: string,
    seedArtist: string,
    endedInfo: TrackInfo | UnresolvedTrackInfo | undefined
) {
    void seedArtist
    ...
    if (
        isSamePrimaryArtist(ca, info.author) &&
        titlesLikelySameSong(ct, info.title, ca, info.author)
    )
        return true
    ...
    if (ea && et && autoplaySameComposition(ca, ct, ea, et)) {
        ...
        if (
            isSamePrimaryArtist(ea, info.author) &&
            titlesLikelySameSong(et, info.title, ea, info.author)
        )
            return true
    }
 ...
 */
function recentSongLine(info: TrackInfo | UnresolvedTrackInfo | undefined) {
    if (!info) return null
 ...
 */
export function autoplayTrackFingerprint(info: TrackInfo | UnresolvedTrackInfo | undefined) {
    if (!info) return ""
 ...
 */
export function rememberAutoplayPlayed(
    player: Player,
    info: TrackInfo | UnresolvedTrackInfo | undefined
) {
 ...
 */
export function clearAutoplayRecent(player: Player) {
 ...
 */
export function seedAutoplayHistoryFromPlayer(player: Player) {
 ...
 */
export function isDuplicateAutoplayCandidate(
    cand: TrackInfo | UnresolvedTrackInfo | undefined,
    ended: TrackInfo | UnresolvedTrackInfo | undefined
) {
...
export function orderSimilarByArtistVariety(
    ...
    const others: { artist: string; title: string }[] = []
    const same: { artist: string; title: string }[] = []
 ...
 */
export function orderLavalinkTracksForAutoplay(
