...
import { getGuildSettings, ensureStorageDir } from "../../util/guildSettings.js"
  ...
  if (player && player.queue && player.queue.current) {
    ...
    embed
      ...
      .addFields(
        {
          name: "Duration",
          value: currentTrack.isStream ? "LIVE" : `${formatDuration(currentTrack.info.duration)}`,
          inline: true,
        },
...
export function createControlButtons(client, player) {
  ...
  client.debug(
    `[ControlHandler] Control buttons created. Button labels: ${row.components.map((c) => c.data.label).join(", ")}`
  )
  return row
}
...
export async function cleanupControlChannel(channel, controlMessageId, client) {
    if (!channel) return
    client.debug(`[ControlHandler] Starting cleanup for channel ${channel.id}`)

    // Check for ManageMessages permission first
    const botPermissions = channel.permissionsFor(client.user)
    if (!botPermissions?.has(PermissionsBitField.Flags.ManageMessages)) {
        client.warn(`[ControlHandler] Missing ManageMessages permission in channel ${channel.id}. Cannot cleanup.`)
        return
    }

    try {
        // Fetch recent messages (limit 15 is usually sufficient and safer)
        const messages = await channel.messages.fetch({ limit: 15 })
        if (messages.size <= 1) {
             client.debug(`[ControlHandler] No messages to clean (or only control message found) in channel ${channel.id}.`)
        ...
        if (messagesToDelete.size > 0) {
            client.debug(`[ControlHandler] Found ${messagesToDelete.size} messages to delete in channel ${channel.id}.`)
            ...
            client.debug(`[ControlHandler] Successfully deleted ${messagesToDelete.size} messages in channel ${channel.id}.`)
        }
        else {
            client.debug(`[ControlHandler] No extraneous messages found to delete in channel ${channel.id}.`)
        }
    } catch (error) {
        // Log errors, potentially common if messages are very old or other issues occur
        client.error(`[ControlHandler] Error during control channel cleanup for channel ${channel.id}:`, error)
        // Avoid spamming warnings for expected permission/deletion issues
        if (!(error.code === 50001 || error.code === 10008 || error.code === 50013 || error.code === 10003)) { // Missing access, Unknown Message, Missing Permissions, Unknown Channel
          console.error(`[ControlHandler] Non-standard error during cleanup:`, error) // Log other unexpected errors
        }
    }
}
...
export async function updateControlMessage(client, guildId) {
    ...
    if (!settings || !settings.controlChannelId || !settings.controlMessageId) {
        client.debug(`[ControlHandler] No control channel/message configured for guild ${guildId}, skipping update.`)
        return
    }
    client.debug(`[ControlHandler] Found settings for guild ${guildId}: Channel ${settings.controlChannelId}, Message ${settings.controlMessageId}`)
    ...
    try {
        ...
        if (!controlChannel || !controlChannel.isTextBased()) {
            client.warn(`[ControlHandler] Control channel ${settings.controlChannelId} not found or not text-based for guild ${guildId}. Cannot update message.`)
            return
        }
        client.debug(`[ControlHandler] Fetched control channel ${controlChannel.id} for guild ${guildId}`)

        const controlMessage = await controlChannel.messages.fetch(settings.controlMessageId).catch(() => null)
        if (!controlMessage) {
            client.warn(`[ControlHandler] Control message ${settings.controlMessageId} not found in channel ${controlChannel.id} for guild ${guildId}. Cannot update message.`)
            return
        }
         client.debug(`[ControlHandler] Fetched control message ${controlMessage.id} for guild ${guildId}`)

        const player = client.lavalink?.getPlayer(guildId)
        client.debug(`[ControlHandler] Fetched player state for guild ${guildId}: ${player ? 'Exists' : 'null'}`)
        ...
        client.debug(`[ControlHandler] Successfully edited control message ${controlMessage.id} in guild ${guildId}`)

        await cleanupControlChannel(controlChannel, controlMessage.id, client)

    } catch (error) {
        if (error.code === 50001 || error.code === 10008 || error.code === 50013) {
            client.warn(`[ControlHandler] Failed to update control message for guild ${guildId} (Code: ${error.code}). Might be missing permissions or message/channel deleted.`)
        } else {
