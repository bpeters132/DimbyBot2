...
function sendAutoplayChannelMessage(client: BotClient, player: Player, line: string) {
  ...
  textCh
    ...
    .catch((e: unknown) => client.error("[LavalinkManager] Failed to send autoplay message:", e))
}
...
async function tryQueueAndPlayAutoplay(
  ...
  const tryOne = async (lavalinkTrack: Track | UnresolvedTrack, label: string) => {
    ...
    if (
      isDuplicateAutoplayCandidate(
        lavalinkTrack.info,
        endedInfo as TrackInfo | undefined
      )
    )
      return false
    ...
    try {
      ...
      client.warn(`[LavalinkManager] Autoplay failed to start "${lavalinkTrack.info?.title}": ${pmsg}`)
      try {
