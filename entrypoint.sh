...
if [ ! -w /app ]; then
  echo "Bot Entrypoint: /app is not writable (uid=$(id -u)); cannot create lavaNodesConfig.js" >&2
  exit 1
fi
...
# Production image runs the entrypoint as root (see Dockerfile): chown writable paths and
# generated lavaNodesConfig.js, then drop to `node` for the bot process.
# Keep this shell as PID 1 so traps can forward shutdown to the bot child process.

if [ "$(id -u)" -eq 0 ]; then
  mkdir -p /app/storage /app/logs /app/downloads
  chown node:node /app 2>/dev/null || true
  chown -R node:node /app/storage /app/logs /app/downloads 2>/dev/null || true
  if [ -f /app/lavaNodesConfig.js ]; then
    chown node:node /app/lavaNodesConfig.js 2>/dev/null || true
  fi
...
mkdir -p /app/storage /app/logs /app/downloads /app/node_modules/.prisma /app/node_modules/@prisma
if [ "$#" -gt 0 ]; then
  trap forward_shutdown INT TERM
